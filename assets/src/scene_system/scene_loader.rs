...
use crate::{event_system::event_manager::get_mut_event_system, save_system::save_system::get_mut_save_system, scene_system::{scene_id::SceneId, scene_template::{Scene, SceneData}}};
...
impl SceneLoader {
    ...
    pub fn unload_scene(&mut self, scene: SceneId) {
        ...
        if loaded_scene.is_some() {
            loaded_scene.unwrap().unload(get_mut_event_system(), get_mut_save_system());
        }
